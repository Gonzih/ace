...
use ace_game_shared::{
    protocol::{Auth, Color, ColorValue, EntityAssignment, Position, Protocol},
    Channels,
};
...
pub fn authorization_event(
    ...
    for event in event_reader.iter() {
        if let AuthorizationEvent(user_key, Protocol::Auth(auth)) = event {
            if *auth.cookie == "donkey-boy" {
                // Accept incoming connection
                info!("donkey-boy has entered the building");
...
pub fn connection_event<'world, 'state>(
    ...
    for event in event_reader.iter() {
        ...
        info!("ACE Server connected to: {}", address);
...
pub fn disconnection_event(
    ...
    for event in event_reader.iter() {
        ...
        info!("ACE Server disconnected from: {:?}", user.address);
